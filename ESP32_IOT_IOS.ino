...
void sendToFirebase(const char* jsonData) {
    ...
    if (httpCode == 200 || httpCode == 204) {
        Serial.println("Sent to Firebase: " + String(jsonData));
    } else if (httpCode > 0) {
        Serial.printf("Firebase send error: %d\n", httpCode);
    } else {
        Serial.printf("Firebase connection error: %s\n", http.errorToString(httpCode).c_str());
    }
...
void firebaseCommunicationTask(void *parameter) {
    Serial.println("[Task2] Firebase Communication Task started on Core 1");
    ...
    while (true) {
        if (wifiConnected && firebaseConfigured) {
            if (!firebaseInitialized) {
                Serial.println("[Task2] Initializing Firebase connection...");
                streamClient.setInsecure();
                connectStream();
                sendIPToFirebase();
                firebaseInitialized = true;
                Serial.println("[Task2] Firebase initialized");
            }
            
            unsigned long currentTime = millis();
            
            if (currentTime - lastSendTime >= FIREBASE_SEND_INTERVAL) {
                if (xSemaphoreTake(dataMutex, 100) == pdTRUE) {
                    if (sharedData.dataReady) {
                        sendToFirebase(sharedData.jsonBuffer);
            ...
            if (currentTime - lastReadTime >= FIREBASE_READ_INTERVAL) {
                int command = readFromFirebase();
                if (command >= 0) {
                    if (xSemaphoreTake(dataMutex, 100) == pdTRUE) {
                        sharedData.commandToAltera = command;
